...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Solves the gridworld maze problem of navigating from the start state to the goal state for the provided gridworld environment')
    parser.add_argument('-plot_files', nargs='?', type=str, help='The file names of results files to load and plot. The files must be present in the Results directory and are specified with a single string, with spaces acting as delimeter between files.')
    ...
    parser.add_argument('-load_models', nargs='?', type=str, help='The file names of a pre-trained model snapshots to load for visualization. Files must be present in the Models directory and are specified with a single string, with spaces acting as delimeter between files., ')
    parser.add_argument('-trial_frequency', nargs='?', type=int, default=5, help='The number of episodes after which to run a trial of the policy learned by the Q-Agent. The default is trial_frequency = 10')
    parser.add_argument('-max', nargs='?', type=int, default=1000, help='The maximum number of steps the agent can take before the episode terminates. The default is max = 1000')
    ...
    parser.add_argument('-e', nargs='?', type=float, default=0.01, help='Epsilon parameter value to be used by the agent when selecting actions epsilon greedy style. Default = 0.01 This represents the minimum value epislon will decay to, since it initially starts at 1')
    parser.add_argument('-a', nargs='?', type=float, default=0.001, help='Alpha parameter which specifies the step size (learning rate) for the update rule. Default value = 0.001')
    ...
    parser.add_argument('-env', choices={'grid', 'continuous_grid'}, type=str, default=GRID, help='Which environment to train the agent on. Options are: {}, {}, {}. Default is env = "grid"'. format(GRID, CONTINUOUS, WINDY))
    ...
    parser.add_argument('--hot', action='store_true', help='Whether to encode the neural network in 1 hot or (row, column) format.')
    parser.add_argument('--visualize', action='store_true', help='Whether to plot the value function, t-SNE, and CCA visualizations for each agent after performing a single run on the environment equal to in length to the epi argument. Default = false')
    parser.add_argument('--q_plot', action='store_true', help='Whether to plot the offline performance of the q policy. Default = false')
